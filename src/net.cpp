...
{
    // Wait for IRC to get it first - disabled with mudcoin
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // mudcoin enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  MudCoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // mudcoin: mint proof-of-stake blocks in the background
