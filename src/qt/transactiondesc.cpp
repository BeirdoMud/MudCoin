...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
                    ...
                    if (wallet->IsMine(txout))
                    {
                        ...
                        if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address))
                        {
                            if (wallet->mapAddressBook.count(address))
                            {
                                ...
                                strHTML += tr("<b>To:</b> ");
                                strHTML += GUIUtil::HtmlEscape(CMudcoinAddress(address).ToString());
        ...
        if (!wtx.mapValue["to"].empty())
        {
            ...
            strHTML += tr("<b>To:</b> ");
            CTxDestination dest = CMudcoinAddress(strAddress).Get();
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(MudcoinUnits::formatWithUnit(MudcoinUnits::MUD, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,-nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += tr("<b>Debit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,-wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += tr("<b>Credit:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,wallet->GetCredit(txout)) + "<br>";
            }
        }

        strHTML += tr("<b>Net amount:</b> ") + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,nNet, true) + "<br>";
        ...
        if (fDebug)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if(wallet->IsMine(txin))
                    strHTML += "<b>Debit:</b> " + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,-wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if(wallet->IsMine(txout))
                    strHTML += "<b>Credit:</b> " + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,wallet->GetCredit(txout)) + "<br>";
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            if (ExtractDestination(vout.scriptPubKey, address))
                            {
                                if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                                    strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                                strHTML += QString::fromStdString(CMudcoinAddress(address).ToString());
                            }
                            strHTML = strHTML + " Amount=" + MudcoinUnits::formatWithUnit(MudcoinUnits::MUD,vout.nValue);
