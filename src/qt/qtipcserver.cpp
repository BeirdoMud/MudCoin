...
// Copyright (c) 20117 MudCoin developers
...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "mudcoin:"))
        {
...
static void ipcThread2(void* pArg)
{
    ...
    message_queue::remove(mudcoinURI_QUEUE_NAME);
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        mq = new message_queue(open_or_create, mudcoinURI_QUEUE_NAME, 2, MAX_URI_LENGTH);

        // Make sure we don't lose any mudcoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one mudcoin instance is listening
        message_queue::remove(mudcoinURI_QUEUE_NAME);
        delete mq;
